...
trigger AccountTrigger on Account(before insert, after insert) {
  ...
  if (Trigger.isBefore && Trigger.isInsert) {
    for (Account acc : Trigger.new) {
      if (acc.Type == null) {
        AccountHelper.setTypeProspect(acc);
      }
       ...
       * Trigger should only fire on insert.
       */
      // copy the shipping address to the billing address if Shipping Street is not null
      if (acc.ShippingStreet != null) {
        AccountHelper.addressCopy(acc);
      }
       ...
       * Trigger should only fire on insert.
       */
      // Set Rating to 'Hot' if phone, website and fax are not null
